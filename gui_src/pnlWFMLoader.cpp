...
//*)
const long pnlWFMLoader::ID_STREAMING_TIMER = wxNewId();
...
{
    wxFileDialog dlg(this, _("Open file"), "", "", "wfm (*.wfm)|*.wfm", wxFD_OPEN | wxFD_FILE_MUST_EXIST);
...
{
    if (UploadFile("lms-suite_wfm/onetone.wfm") < 0)
...
void pnlWFMLoader::OnbtnLoadFileClick(wxCommandEvent& event)
{
    wxFileDialog dlg(this, _("Open file"), "", "", "wfm (*.wfm)|*.wfm", wxFD_OPEN | wxFD_FILE_MUST_EXIST);
    if (dlg.ShowModal() == wxID_CANCEL)
        return;
    vector< pair<int, int> > iq_pairs;
    if (ReadWFM(dlg.GetPath().c_str(), "output.txt", iq_pairs) < 0)
    {
        wxMessageBox(wxString::Format("File not found %s", dlg.GetPath().c_str()), "Error");
        return;
    }
    int bufferSize = iq_pairs.size() * sizeof(short);
    unsigned short *buffer = new unsigned short[bufferSize];
    memset(buffer, 0, bufferSize);
    int bufPos = 0;
    short ii = 0;
