...
#define HW_LSTREAMER L"Stream"
#define HW_LLMS_DEV_LMS6002USB L"LMS6002-USB"

#define HW_DIGIRED "DigiRed"
#define HW_DIGIGREEN "DigiGreen"
#define HW_STREAMER "Stream"
#define HW_LMS_DEV_LMS6002USB "LMS6002-USB"
...
bool ConnectionUSB::Open(int index)
{
	...
	if( index < USBDevicePrimary->DeviceCount())
	{
		if(USBDevicePrimary->Open(index))
		{
            ...
            else if (m_hardwareDesc.find(HW_LLMS_DEV_LMS6002USB, 0) != wstring::npos)
                m_hardwareName = HW_LMS_DEV_LMS6002USB;
            else
                m_hardwareName = HW_DIGIGREEN;
            if(m_hardwareName == HW_DIGIRED || m_hardwareName == HW_STREAMER || m_hardwareName == HW_LMS_DEV_LMS6002USB) //digired, controls through control end point
            {
...
int ConnectionUSB::SendData(const unsigned char *buffer, unsigned long length, unsigned long timeout_ms)
{
    ...
    if(IsOpen())
    {
        if(m_hardwareName == HW_DIGIRED || m_hardwareName == HW_STREAMER || m_hardwareName == HW_LMS_DEV_LMS6002USB)
        {
            #ifndef __unix__
            if(OutCtrlEndPt3)
            {
...
eLMS_DEV ConnectionUSB::ConnectedDeviceType()
{
    ...
    else if(m_hardwareName == HW_LMS_DEV_LMS6002USB)
        return LMS_DEV_LMS6002USB;
    else
