...
unsigned char* PreparePacket(const GenericPacket &pkt, int &length, const eLMS_PROTOCOL protocol, ConnectionManager* pSerPort)
{
    ...
    if(protocol == LMS_PROTOCOL_LMS64C)
    {
        ...
        if(packet.cmd == CMD_LMS7002_RD || packet.cmd == CMD_LMS6002_RD || packet.cmd == CMD_BRDSPI16_RD)
            maxDataLength = maxDataLength/2;
        packet.blockCount = pkt.outLen/blockRatio;
        int bufLen = packet.blockCount/(maxDataLength/blockRatio)
                    +(packet.blockCount%(maxDataLength/blockRatio)!=0);
        ...
        for(int j=0; j*packet.pktLength<bufLen; ++j)
        {
            ...
            if(packet.blockCount > (maxDataLength/blockRatio))
            {
                buffer[pktPos+2] = maxDataLength/blockRatio;
            ...
            int bytesToPack = (maxDataLength/blockRatio)*blockRatio;
            memcpy(&buffer[pktPos+8], &pkt.outBuffer[srcPos], bytesToPack);
            unsigned short textVal = 0xABCD;
            bool mSystemBigEndian = ((unsigned char*)&textVal)[0] == 0xAB;
            if( !mSystemBigEndian && (pkt.cmd == CMD_LMS7002_WR || pkt.cmd == CMD_LMS7002_RD || pkt.cmd == CMD_BRDSPI16_WR || pkt.cmd == CMD_BRDSPI16_RD))
            {
                for(int i=pktPos+8; i<pktPos+8+bytesToPack; i+=2)
                {
...
int ParsePacket(GenericPacket &pkt, const unsigned char* buffer, const int length, const eLMS_PROTOCOL protocol)
{
    ...
    if(protocol == LMS_PROTOCOL_LMS64C)
    {
        ...
        unsigned short textVal = 0xABCD;
        bool mSystemBigEndian = ((unsigned char*)&textVal)[0] == 0xAB;
        if( !mSystemBigEndian && (pkt.cmd == CMD_LMS7002_WR || pkt.cmd == CMD_LMS7002_RD || pkt.cmd == CMD_BRDSPI16_RD || pkt.cmd == CMD_BRDSPI16_WR))
        {
            for(int i=0; i<pkt.inLen; i+=2)
            {
                unsigned char val = pkt.inBuffer[i];
                pkt.inBuffer[i] = pkt.inBuffer[i+1];
                pkt.inBuffer[i+1] = val;
            }
        }
    }
